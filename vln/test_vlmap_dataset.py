...
sys.path.append(os.path.join((os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "vlmaps"))
print(sys.path)
...
class IsaacSimLanguageRobot(LangRobot):
    ...
    # def load_scene_map(self, vlmaps_data_dir: str, map_config: DictConfig):
    #     self.map = Map.create(map_config)
    #     self.map.load_map(vlmaps_data_dir)
    ...
    def turn(self, angle_deg: float):
        ...
        """
        # for habitat it turns 7 degrees each time, so turning 50 degrees needs 7 turns 
        # if angle_deg < 0:
