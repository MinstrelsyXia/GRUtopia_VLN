...
class CMA_DP_noRNN_Net(nn.Module):
    def __init__(
        ...
        if self.model_config.Diffusion_Policy.type == 'resnet_unet':
            ...
            # condition: [cur_obs, his_obs, txt, prev_act, step, imu]
            obs_length = 1
            if self.model_config.IMAGE_ENCODER.use_stack:
                his_length = self.model_config.IMAGE_ENCODER.img_stack_nums
            else:
            ...
            txt_length = self.model_config.TEXT_ENCODER.q_former_length if self.model_config.TEXT_ENCODER.use_qformer else self.model_config.TEXT_ENCODER.max_length
            ...
            n_obs_steps = obs_length + his_length + txt_length + prev_act_length + step_length + imu_length
            ...
            self.action_dp_pred_net = TransformerForDiffusion(
                    ...
                    cls_mask_ratio=self.model_config.Diffusion_Policy.cls_mask_ratio
                )
        ...
        # Init the self-attn net for auxiliary tasks
        self.self_attn_net = encoders.RobertaSelfAttention(cross_modal_config) # hidden_size=512
    ...
    def pred_actions(
        ...
        prev_actions = prev_actions.float() # [bs, act_length, 3]
        ...
        if denoise_action:
            ...
            if self.dp_type == 'transformer':
                ...
                if self.model_config.STEP_ENCODER.use:
                    lv_state = torch.cat([lv_state, steps_embeds],dim=1)
                if self.model_config.IMU_ENCODER.use:
                    lv_state = torch.cat([lv_state, imu_embeds], dim=1)

                type_embeds = ([0] * 1 +  # cur_obs
                            [1] * img_txt_embeds.shape[1] +  # his_obs
                            [2] * fused_update_txt_embeds.shape[1] +  # txt
                            [3] * prev_action_embeds.shape[1])  # prev_act
                
                if self.model_config.STEP_ENCODER.use:
                    type_embeds += [4] * steps_embeds.shape[1]
                if self.model_config.IMU_ENCODER.use:
                    type_embeds += [5] * imu_embeds.shape[1]
