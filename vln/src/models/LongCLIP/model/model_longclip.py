...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, proj=True):
        x = self.conv1(x)  # shape = [*, width, grid, grid] # [bs, 3, 224, 224]
    ...
    def forward_multiple_patches(self, x: torch.Tensor):
        ...
        avg_pool_features = F.avg_pool2d(patch_features_permute, kernel_size=(grid_num_down,grid_num_down), stride=(grid_num_down, grid_num_down))
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, proj=True):
        return self.visual(image.type(self.dtype), proj=proj)
    
    def encode_image_multi_patches(self, image):
        return self.visual.forward_multiple_patches(image.type(self.dtype))
