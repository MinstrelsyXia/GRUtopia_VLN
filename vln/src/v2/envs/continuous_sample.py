...
from vln.src.v2.util.path_plan import plan_and_get_actions_continuous
import numpy as np
from copy import deepcopy


class ContinuousSampleSingleScanEnv(BaseSingleScanEnv):
    ...
    def execute_one_action(
        ...
        while not finish_state:
            need_sample = self.step % sample_interval == 0
            if need_sample:
    ...
    def sample(self):
        ...
        for path_key in sample_path_key_list:
            ...
            while True:
                ...
                robot_position, robot_rotation = self.task.get_robot_poses_without_offset()
                # path_plan
                exe_path, find_flag, reason = plan_and_get_actions_continuous(
                    map_info=map_info,
                ...
                if not find_flag or exe_path is None or len(exe_path) == 0:
                    finish = True
                ...
                action_success, fail_reason = self.execute_one_action(
                    exe_path,
                    stuck_checker,
                    data_collector,
                    current_point_index / len(nav_path),
                )
                log.info(f"[scan:{scan}][path:{path_key}] finish one action[step:{self.step}][ {current_point_index + 1} / {len(nav_path)} ][result:{fail_reason}]")
