...
def get_release_scans(release_file):  
    # 使用urlopen打开URL，并获取HTTPResponse对象  
    response = urllib.request.urlopen(release_file)  
    # 读取所有内容，并转换为字符串（假设内容是UTF-8编码的）  
    scan_content = response.read().decode('utf-8')  
    # 分割字符串为行列表  
    scan_lines = scan_content.splitlines()  
    # 移除每行末尾的换行符（如果有的话）  
    scans = [scan_line.rstrip('\n') for scan_line in scan_lines]  
    return scans 
