...
sys.path.pop(0)
sys.path.append('/wangliuyi/isaac-sim-4.0.0/GRUtopia_wensi')
import json
import yaml
import isaacsim
...
class SimulatorConfig_evaluate(SimulatorConfig):
    def __init__(self, config_dict):
        ...
        self.config = None
        self.validate()
from tao_yuan.core.runner import SimulatorRunner
...
def main():
    ...
    config_dict = load_file(os.path.join(ROOT_DIR, "llm_agent/evaluate.yaml"))
    ...
    # initialize agent
    agent_config_dict = os.path.join(ROOT_DIR, 'llm_agent/agent/agent_config.yaml')
    with open(agent_config_dict, 'r') as file:
    ...
    # scenes data path
    data_path = os.path.join(ROOT_DIR, 'evaluate_data/data/meta')
    scenes_path = os.path.join(ROOT_DIR, 'evaluate_data/splits', args.mode, 'scenes')
    ...
    # result path
    if agent_type.split('_')[0] == 'agent':
        result_path = os.path.join(ROOT_DIR, 'result', agent_type + agent_llm + '_' + str(args.dialogue_turn))
    elif agent_type.split('_')[0] == 'baseline':
        result_path = os.path.join(ROOT_DIR, 'result', agent_type + agent_vlm + '_' + str(args.dialogue_turn))
    ...
    ###############################################################################################################
    # object data
    object_caption = load_file(os.path.join(ROOT_DIR, 'evaluate_data/splits/all/object_captions_score_sort.json'))
    object_caption_embeddings = load_file(os.path.join(ROOT_DIR, 'evaluate_data/splits/all/object_captions_embeddings.pkl'))
