from vlmaps.vlmaps.GLEE.glee.models.glee_model import GLEE_Model
from vlmaps.vlmaps.GLEE.glee.config_deeplab import add_deeplab_config
from vlmaps.vlmaps.GLEE.glee.config import add_glee_config
import matplotlib.pyplot as plt
import numpy as np

# from habitat_sim.utils.common import d3_40_colors_rgb
d3_40_colors_rgb = (plt.cm.tab20(np.linspace(0, 1, 20))[:, :3] * 255).astype(np.uint8)
d3_40_colors_rgb = np.vstack([
    d3_40_colors_rgb,
    (plt.cm.tab20b(np.linspace(0, 1, 20))[:, :3] * 255).astype(np.uint8)
])
...
def initialize_glee(glee_config=GLEE_CONFIG_PATH,
                    ...
                    device="cuda:1"):
    # cfg_swin = OmegaConf.load(glee_config)
    cfg_swin = get_cfg()
