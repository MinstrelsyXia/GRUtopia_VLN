...
do
    ...
    if [ -f "$LAST_SCAN_FILE" ]; then
        ...
        # 在后台运行 Python 脚本
        nohup python $SCRIPT_PATH --config-name vlmap_dataset_cfg resume_scan=$LAST_SCAN episode_file=$GPU_LIST_FILE last_scan_file=$LAST_SCAN_FILE > $LOG_FILE 2>&1 & 
        PID=$!
    else
        echo "GPU ${GPU_IDX}: Starting fresh run"
        nohup python $SCRIPT_PATH --config-name vlmap_dataset_cfg episode_file=$GPU_LIST_FILE last_scan_file=$LAST_SCAN_FILE> $LOG_FILE 2>&1 &
        PID=$!
    fi
