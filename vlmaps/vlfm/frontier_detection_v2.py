...
from vlmaps.vlfm.utils import bresenhamline
from vlmaps.vlfm.utils import closest_line_segment
...
def detect_frontier_waypoints(
    ...
    '''
    return:
        frontier point: in matrix coord
        frontier angle: in matrix coord, [0,2*np.pi]
    '''
    if get_grad == False:
        if DEBUG:
            import time
        ...
        for idx, waypoint in enumerate(waypoints):
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    check_x = mid_x + dx
                    check_y = mid_y + dy
                    if (0 <= check_x < explored_mask.shape[0] and 
                        0 <= check_y < explored_mask.shape[1]):
                        if explored_mask[check_x, check_y] > 0:
                            nx += dx
                            ny += dy
            ...
            # angle = angle % (2*np.pi) #[0,2*np.pi]
            frontier_angles.append(angle+np.pi)
    ...
    if my_visualize:
        ...
        cv2.imwrite("tmp/frontiers_with_grad_and_points.png", img)
