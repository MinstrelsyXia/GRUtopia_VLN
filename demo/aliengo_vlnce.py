# Author: w61
# Date: 2024.7.11
''' Demo for loading vlnce dataset and intialize the Aliengo robot
'''
import os,sys
import gzip
import json
...
parser = argparse.ArgumentParser(description="Demo for loading vlnce dataset and intialize the Aliengo robot.")
...
file_path = 'demo/configs/aliengo_locomotion.yaml'
...
def load_data(file_path, path_id=None, verbose=False):
    ...
    ref_path = []
    for path in target_item['reference_path']:
        new_path = [path[0], -path[2], path[1]+1.05]
        ref_path.append(new_path)
    target_item['reference_path'] = ref_path
...
path = data_item['reference_path']

actions = {'aliengo': {'move_along_path': [path]}}
