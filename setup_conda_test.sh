...
# Choose venv name
echo "The new conda environment will be named grutopia by default."
...
# 获取Python版本 - 使用更强大的方式提取版本号
ISAAC_PYTHON_VERSION=$(${ISAAC_SIM_PATH}/python.sh -c "import platform; print(platform.python_version())" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | tail -1)

# 如果未能正确获取版本，使用默认版本
if [ -z "$ISAAC_PYTHON_VERSION" ]; then
    echo "无法从Isaac Sim获取Python版本，使用默认版本3.10"
